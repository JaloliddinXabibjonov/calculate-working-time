...
class RoleResourceIT {
    ...
    private static final RoleName DEFAULT_NAME = RoleName.BOSS;
    private static final RoleName UPDATED_NAME = RoleName.USER;
    ...
    @Test
    @Transactional
    void getAllRolesByWorkerIsEqualToSomething() throws Exception {
        // Initialize the database
        roleRepository.saveAndFlush(role);
        Worker worker;
        ...
        // Get all the roleList where worker equals to workerId
        defaultRoleShouldBeFound("workerId.equals=" + workerId);

        // Get all the roleList where worker equals to (workerId + 1)
        defaultRoleShouldNotBeFound("workerId.equals=" + (workerId + 1));
    }
