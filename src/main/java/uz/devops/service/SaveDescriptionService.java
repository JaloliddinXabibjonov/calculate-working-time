...
@Service
public class SaveDescriptionService extends AbstractBot {
    ...
    protected void process(Update update, Worker worker, boolean checkBoss, SendMessage sendMessage) {
        reasonDescriptionService.save(checkBoss, worker.getId(), sendMessage, update.getMessage().getText().substring(5));
    }
}
